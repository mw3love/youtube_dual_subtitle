...
- **프롬프트는 사용자 편집 가능**: `settings.explainPrompt`(storage.sync), 기본값 `defaultExplainPrompt(uiLang)`(`settings.ts` → i18n 사전의 `prompt.explainDefault`, 원형은 사용자 Gem 프롬프트. 표시 언어별로 「답변 언어」가 다른 두 벌 — 섹션 46). 옵션 페이지 textarea + "기본값으로" 버튼. system 메시지로 그대로 전달돼 답변 형식을 정함.
...
- **AI 답변 언어는 별개 축이 아니라 이 설정을 따라간다:** 해설/질문 기본 프롬프트에 "답변은 한국말로"가 박혀 있어 UI만 영어로 바꾸면 영어 UI에 한국어 해설이 나온다. `defaultExplainPrompt(lang)`/`questionSystemPrompt(lang)`으로 두 벌을 두고, **언어를 전환할 때 저장된 `explainPrompt`가 「직전 언어의 기본값 그대로」면 새 언어 기본값으로 갈아끼운다**(`options/main.tsx:onUiLangChange`). 사용자가 편집한 프롬프트는 건드리지 않는다 — 그쪽이 사용자 자산이라 덮어쓰면 복구가 안 된다.
...
- **개인정보처리방침은 저장소 루트 `PRIVACY.md`**(공개 repo에 커밋, GitHub blob URL을 웹스토어 "개인정보 보호" 탭의 개인정보처리방침 URL에 사용). 웹스토어 "사용자 데이터 사용"에서 "웹사이트 콘텐츠"(자막 텍스트가 번역/해설을 위해 외부 API로 전송됨)를 체크하면 필수가 되는 필드 — BYOK 백엔드가 추가/변경되면 이 파일의 "자막 텍스트의 처리" 절도 함께 갱신.
