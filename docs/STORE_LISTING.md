...
✓ 결정됨 — `Dual Subtitle for YouTube`. 옛 `YouTube Dual Subtitle`은 Google 자사 상표(`YouTube`)를 이름 맨 앞에 그대로 써서 "공식 제휴처럼 보이지 않는가"가 심사관 재량 판단이라 리네임 요청을 받을 리스크가 있었음 — 상표를 뒤로 보내고 `for`로 비공식임을 명확히 하는 쪽으로 `manifest.ts`의 `name`과 문서 전체 표기를 교체 완료.
...
```
Dual Subtitle for YouTube는 영상에 원문 자막과 번역 자막을 동시에 표시합니다. 외국어 영상을 보면서 모르는 단어만 번역으로 확인하고 싶을 때, 또는 어학 학습용으로 적합합니다.

■ 주요 기능
- 듀얼 자막: 원문(영어 등) + 번역(한국어 등) 두 줄 동시 표시
- 표시 언어: 영어/한국어 — 설치 시 브라우저 언어를 따르고 옵션에서 전환 (AI 해설 기본 프롬프트의 답변 언어도 함께 따라감). 번역 언어도 설치 시 브라우저 언어로 시작
...
- [x] 확장 이름 최종 결정 — `Dual Subtitle for YouTube` (위 1-b 참고)
