...
| 2.4 | sourceLang=일본어 설정 + 일본어 영상 | 일본어 트랙 잡힘 |

## 3. 토글·UI

| # | 시나리오 | 통과 기준 |
|---|---|---|
| 3.1 | C 키 토글 | 자막 표시/숨김 + native CC 버튼 시각 동기 |
| 3.2 | 검색창에 'c' 입력 | 자막 토글 발화 X (input focus 보호) |
...
| 3.6 | native CC 버튼 직접 클릭으로 끔 | 우리 자막은 계속 표시 (단방향 sync 의도) |
| 3.7 | native CC 버튼으로 켬 (off 상태에서) | 우리 자막도 ON 동기화 |
