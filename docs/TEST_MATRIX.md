...
| 4.3a | 텍스트 선택을 박스 밖(아래 채널바·옆 헤더)까지 끌기 — Shorts 하단 위치 포함 (A71) | 선택이 자막 박스 안에 머물고 해설/질문 툴바가 뜸 |
| 4.3b | Gemini/게이트웨이 번역 진행 중 자막 선택 (A71) | 뒤 문장 번역이 도착해도 선택·툴바 유지. 선택 중엔 자막이 현재 문장에 고정, 해설 클릭 후 따라잡음 |
...
| # | 시나리오 | 통과 기준 |
|---|---|---|
| 7.1 | 옵션 페이지 로드 | settings 모든 항목 표시. "Loading options…"(ko면 "옵션 불러오는 중…") 잠깐 보이고 사라짐 |
| 7.2 | 미리보기 박스 | 외국어 박스 = displayMode 반영. 모국어 박스 = source-only |
| 7.3 | 옵션 초기화 | 확인 → 모든 값 default(표시 언어·번역 언어·해설 프롬프트는 브라우저 언어 기준, A72). 캐시는 유지 |
| 7.4 | Gemini 섹션은 backend=gemini 일 때만 노출 | google-free 선택 시 Gemini 섹션 숨김 |
| 7.5 | 단축키 표시 | 저장된 토글 키가 "Show subtitles"/"자막 켜기" 라벨 옆에 표시 |
| 7.6 | 표시 언어 전환 (A70) | Language를 한국어로 → 옵션·팝업 문구가 즉시 한국어. 다시 English로 되돌아옴 |
| 7.7 | 표시 언어 ↔ 해설 프롬프트 (A70) | 프롬프트 미편집 상태에서 언어 전환 시 해설 프롬프트 기본값도 그 언어로 교체("기본값으로" 버튼 비활성 유지). 편집한 프롬프트는 전환해도 그대로 |
| 7.8 | 해설 패널 라벨 재적용 (A70) | 패널을 연 채 언어 전환 → 💡/❓/📋/📝/🖍️ 버튼 라벨이 새 언어로 바뀜(탭 본문 답변은 그대로). 아직 질문 안 보낸 직접 질문 탭의 제목·안내문도 바뀜(A72) |
| 7.9 | 새 설치 언어 시드 (A72) | 새 프로필에 설치 → 브라우저 언어 ko면 표시·번역 언어 ko, en이면 en/en, ja면 표시 en·번역 ja. chrome://extensions 설명·단축키 설명도 브라우저 언어(en/ko) |
| 7.10 | AI 답변 언어 = 번역 언어 (A73) | 표시 English·번역 Español로 💡 해설/❓ 질문 → 답변이 스페인어. 옛 기본 프롬프트가 저장된 상태에서도 "Restore default" 버튼 비활성 |
